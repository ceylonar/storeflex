...
export function PurchaseReceipt({ purchase, onNewPurchase }: PurchaseReceiptProps) {
    ...
    <div className="bg-background text-foreground">
      ...
      <div id="printable-receipt" ref={receiptRef} className="p-6 text-sm font-mono">
          ...
          <tbody>
            {purchase.items.map(item => (
              <tr key={item.id} className="border-b border-dotted">
                ...
                <td className="text-right py-1">{item.cost_price.toFixed(2)}</td>
                <td className="text-right py-1">{item.total_cost.toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="mt-4 ml-auto max-w-[150px] text-xs space-y-1">
          ...
          <div className="flex justify-between"><span>Tax ({purchase.tax_percentage}%)</span> <span>{purchase.tax_amount.toFixed(2)}</span></div>
